...
namespace App\Actions\Platform\Setup;
...
class CreateTenantAction
{
    ...
    public function validate(array $input)
    {
        ...
        if (filter_var($input['domain'], FILTER_VALIDATE_DOMAIN, FILTER_FLAG_HOSTNAME) === false) {
            throw ValidationException::withMessages(['domain' => 'The domain is not a valid domain.']);
        }

        // make sure domain does not contain multiple dots
        if (substr_count($input['domain'], '.') > 2) {
            throw ValidationException::withMessages(['domain' => 'The domain should be of format subdomain.trimy.nl or subdomain.trimy.net.']);
        }

        // get the subdomain from the domain and check if it is not a banned subdomain name
        $subdomain = explode('.', $input['domain'])[0];
        if (in_array($subdomain, $this->bannedSubdomainNames())) {
            throw ValidationException::withMessages(['domain' => 'The subdomain is not allowed.']);
        }

        return Validator::make($input, [
            'name' => ['required', 'string', 'max:50'],
            'domain' => ['required', 'string', 'max:50', 'ends_with:.trimy.nl,.trimy.net', 'unique:tenants,domain'],
            'language' => ['required', 'string', 'max:5'],
            'timezone' => ['required', 'string', Rule::in(World::timezones())],
            'clock' => ['required', 'string', Rule::in(['12', '24'])],
